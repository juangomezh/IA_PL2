  ...
  (for ([i (in-range 0 64)])
    (when (not (equal? (list-ref tablero i) 'libre))
      ((draw-solid-ellipse ventana)
       ...
       30	 	 	 	 
       (getColor tablero i)))))
;_______________________________________________________________________PRUEBAS UNITARIAS____________________________________________________________________________;;
...
(check-match (voltear tab 20 'blanc 8) empty)
...
(let ([res (cons 0 -1)])
  (check-match (alphaB tab 'blanc -inf.0 +inf.0 0) cons))
